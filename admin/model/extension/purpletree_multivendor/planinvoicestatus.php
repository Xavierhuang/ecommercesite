...
class ModelExtensionPurpletreeMultivendorPlaninvoicestatus extends Model {
		...
		public function addSubscriptionplaninvoicestatus($data) {		
			...
			foreach ($data['subscriptionplan_invoice_status'] as $language_id => $value) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "purpletree_vendor_plan_invoice_status_l SET status_id = '" . (int)$status_id . "', language_id = '" . (int)$language_id . "', status = '" . $this->db->escape($value['invoice_status']) ."'");
			}
		}
		
		public function editSubscriptionplaninvoicestatus($status_id, $data) {		
			$this->db->query("DELETE FROM " . DB_PREFIX . "purpletree_vendor_plan_invoice_status_l WHERE status_id = '" . (int)$status_id . "'");
			foreach ($data['subscriptionplan_invoice_status'] as $language_id => $value) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "purpletree_vendor_plan_invoice_status_l SET status_id = '" . (int)$status_id . "', language_id = '" . (int)$language_id . "', status = '" . $this->db->escape($value['invoice_status']) ."'");
			}		
		}
		...
		public function getSubscriptionplaninvoicestatus($data) {	
			...
			$sql = "SELECT DISTINCT pvpis.*,pvpisl.status AS invoice_status FROM " . DB_PREFIX . "purpletree_vendor_plan_invoice_status pvpis JOIN " . DB_PREFIX . "purpletree_vendor_plan_invoice_status_l pvpisl ON (pvpis.status_id = pvpisl.status_id) WHERE  pvpisl.language_id = '" . (int)$this->config->get('config_language_id') . "'";		
		...
		public function getInvoicestatus($status_id) {
			...
			$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "purpletree_vendor_plan_invoice_status_l WHERE status_id = '" . (int)$status_id . "'");
