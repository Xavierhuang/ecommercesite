...
class ModelExtensionPurpletreeMultivendorBulkproductupload extends Model{
	...
	public function checkExistingProduct($product_column,$value) {
		...
		$query=$this->db->query("SELECT * FROM " . DB_PREFIX . "product WHERE ".$product_column." = '".$value."'");
			if($query->num_rows){
				return true;
			}
			return false;
	...
	public function checkExistingProductIds($product_column,$value) {
		...
		$query=$this->db->query("SELECT product_id FROM " . DB_PREFIX . "product WHERE ".$product_column." = '".$value."'");
		...
		public function editProductDataTab($data) {
		  if($data['shipping_charge'] == ''){
			...
			$this->db->query("UPDATE " . DB_PREFIX . "product SET model = '" . $this->db->escape($data['model']) . "', sku = '" . $this->db->escape($data['sku']) . "', upc = '" . $this->db->escape($data['upc']) . "', ean = '" . $this->db->escape($data['ean']) . "', jan = '" . $this->db->escape($data['jan']) . "', isbn = '" . $this->db->escape($data['isbn']) . "', mpn = '" . $this->db->escape($data['mpn']) . "', location = '" . $this->db->escape($data['location']) . "', quantity = '" . (int)$data['quantity'] . "', minimum = '" . (int)$data['minimum'] . "', subtract = '" . (int)$data['subtract'] . "', stock_status_id = '" . (int)$data['stock_status_id'] . "', date_available = '" . $this->db->escape($data['date_available']) . "', manufacturer_id = '" . (int)$data['manufacturer_id'] . "', shipping = '" . (int)$data['shipping'] . "', shipping_charge=".$shipping_charge.", price = '" . (float)$data['price'] . "', points = '" . (int)$data['points'] . "', weight = '" . (float)$data['weight'] . "', weight_class_id = '" . (int)$data['weight_class_id'] . "', length = '" . (float)$data['length'] . "', width = '" . (float)$data['width'] . "', height = '" . (float)$data['height'] . "', length_class_id = '" . (int)$data['length_class_id'] . "', status = '" . (int)$data['status'] . "', tax_class_id = '" . (int)$data['tax_class_id'] . "',image ='". $this->db->escape($data['image']) ."', sort_order = '" . (int)$data['sort_order'] . "', date_added = NOW(), date_modified = NOW() WHERE product_id = '" . $this->db->escape($data['product_id']) . "'");
