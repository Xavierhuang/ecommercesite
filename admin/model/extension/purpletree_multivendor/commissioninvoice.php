...
class ModelExtensionPurpletreeMultivendorCommissioninvoice extends Model{
		...
		public function getOrderProducts($commisionid,$order_id,$product_id,$seller_id){
		    $vendor_order_table_id = '';
			$sql = "SELECT vendor_order_table_id FROM " . DB_PREFIX . "purpletree_vendor_commissions WHERE id = '".(int)$commisionid."'";
			$query  = $this->db->query($sql);
			if ($query->num_rows) {
				$vendor_order_table_id = $query->row['vendor_order_table_id'];
			}else{
			$sqll = "SELECT vendor_order_table_id FROM " . DB_PREFIX . "purpletree_vendor_commissions WHERE order_id = '".(int)$order_id."' AND product_id = '".(int)$product_id."' AND seller_id = '".(int)$seller_id."'";
			 $queryy  = $this->db->query($sqll);
			 $vendor_order_table_id = $queryy->row['vendor_order_table_id'];
			}			
			if($vendor_order_table_id !== null){
			$sql = "SELECT pvo.*,pd.name,op.* FROM " . DB_PREFIX . "purpletree_vendor_orders pvo JOIN " .DB_PREFIX. "product_description pd ON(pd.product_id=pvo.product_id) JOIN " .DB_PREFIX. "order_product op ON(op.order_id=pvo.order_id AND op.price=pvo.unit_price) ";
			$sql .= " WHERE pvo.id ='".(int)$vendor_order_table_id."'";
		...
		public function getDefaultstatus(){
			...
			$sql = "SELECT status FROM " . DB_PREFIX . "purpletree_vendor_plan_invoice_status_l WHERE status_id = '1'";
		...
		public function getCommissions($data=array()){
			...
			$sql = "SELECT pvc.id,pvc.created_at,pvs.store_name FROM `" . DB_PREFIX . "purpletree_vendor_commission_invoice` pvc LEFT JOIN `". DB_PREFIX ."purpletree_vendor_commission_invoice_items` pvcii ON (pvc.id = pvcii.link_id )LEFT JOIN `". DB_PREFIX ."purpletree_vendor_stores` pvs ON (pvcii.seller_id = pvs.seller_id ) RIGHT JOIN `" . DB_PREFIX . "order` o ON(o.order_id = pvcii.order_id) WHERE pvc.id != ''";
			if ($data['seller_id']!=0) {
		...
		public function editCommissionHistory($data) {
			...
			$query4 = $this->db->query("SELECT status_id FROM ". DB_PREFIX ."purpletree_vendor_plan_invoice_status_l WHERE status='".$this->db->escape($data['status'])."' AND language_id='".(int)$this->config->get('config_language_id')."'"); 
			if ($query4->num_rows) {
		...
		public function getCommissionInvoiceStatus() {
			$query = $this->db->query("SELECT status_id,status FROM ". DB_PREFIX ."purpletree_vendor_plan_invoice_status_l WHERE language_id='".(int)$this->config->get('config_language_id')."'");
			if ($query->num_rows) {
				return $query->rows;
			}
			return false;
		...
		public function getCommissionStatus($status_id) {
			$query = $this->db->query("SELECT status FROM ". DB_PREFIX ."purpletree_vendor_plan_invoice_status_l WHERE status_id='".(int)$status_id."' AND language_id='".(int)$this->config->get('config_language_id')."'"); 
			if ($query->num_rows) {
				return $query->row['status'];
			}
			return false;
