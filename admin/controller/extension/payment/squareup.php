...
class ControllerExtensionPaymentSquareup extends Controller {
    ...
    public function connect() {
        ...
        if($this->request->post['payment_squareup_key_type'] == "sandbox"){
          if(strpos($this->request->post['payment_squareup_client_id'], "sandbox") === false || strpos($this->request->post['payment_squareup_client_secret'], "sandbox") === false){
            $json['error'] = "Sandbox Application ID Key and Application Secret Key should contain the word sandbox at the begining.";
          }
        }

        if($this->request->post['payment_squareup_key_type'] == "production"){
          if(strpos($this->request->post['payment_squareup_client_id'], "sandbox") !== false || strpos($this->request->post['payment_squareup_client_secret'], "sandbox") !== false){
        ...
        if (empty($json['error'])) {
            ...
            $this->session->data['payment_squareup_connect']['payment_squareup_key_type'] = $this->request->post['payment_squareup_key_type'];

            $json['redirect'] = $this->squareup_api->authLink($this->request->post['payment_squareup_client_id'], $this->request->post['payment_squareup_key_type']);
        }
    ...
    public function transaction_info() {
        ...
        if($this->config->get('payment_squareup_key_type') == "sandbox"){
          $data['url_transaction'] = sprintf(
              Squareup::SANDBOX_VIEW_TRANSACTION_URL,
              $transaction_info['transaction_id'],
              $transaction_info['location_id']
          );
        }
        ...
        if ($api_info && $this->user->hasPermission('modify', 'sale/order')) {
            ...
            // OC 3.0.3.7 fixes spelling function name below causing issues
            if(property_exists($this->model_user_api, "deleteApiSessionBySessonId")){
    ...
    protected function acceptNewMerchant($force_on_demand_sync = false) {
        ...
        $previous_setting['payment_squareup_key_type'] = $this->session->data['payment_squareup_connect']['payment_squareup_key_type'];
    ...
    public function refund() {
        $this->transactionAction(function($transaction_info, &$json) {
            ...
            $bogi = array('id' => $transaction_info['squareup_transaction_id'], 'status' => $status, 'refunds' => $refunds);
    ...
    protected function showForm() {
        ...
        try {
            ...
            if ($this->config->get('payment_squareup_access_token')) {
                if (false === $locations = $this->squareup_api->verifyToken($this->config->get('payment_squareup_access_token'), $first_location_id)) {
                    ...
                    $previous_setting['payment_squareup_merchant_name'] = $this->squareup_api->getMerchantName($previous_setting['payment_squareup_merchant_id']);
                }
            }
        ...
        $data['payment_squareup_key_type']              = $this->getSettingValue('payment_squareup_key_type');
