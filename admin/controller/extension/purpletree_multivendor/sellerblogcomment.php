...
class ControllerExtensionPurpletreeMultivendorSellerblogcomment extends Controller {
		...
		protected function getForm() {
			...
			$data['text_select_post'] = $this->language->get('text_select_post');
			...
			$sellerBlogs = array();
			$sellerBlogs = $this->model_extension_purpletree_multivendor_sellerblogpost->getBlogs($filter_data);
			if(!empty($sellerBlogs)){
			foreach ($sellerBlogs as $sellerBlog) {
					
					$data['sellerBlogPosts'][] = array(
					'blog_post_id' => $sellerBlog['blog_post_id'],
					'title'       => strip_tags(html_entity_decode($sellerBlog['title'], ENT_QUOTES, 'UTF-8'))
					);
				}
			}
