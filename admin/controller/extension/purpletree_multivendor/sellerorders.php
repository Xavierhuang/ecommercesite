...
class ControllerExtensionPurpletreeMultivendorSellerorders extends Controller {
		...
		protected function getList() {
			...
			if (isset($this->request->get['seller_id_filter'])) {
				...
				//$data['seller_id_filter'] = $this->request->get['seller_id_filter'];
				} else {
			...
			$data['helplink'] = "https://cutt.ly/sCoV015";
			...
			if (!$this->customer->validateSeller()) {
				...
				$data['invoice'] = $this->url->link('extension/purpletree_multivendor/sellerorders', 'user_token=' . $this->session->data['user_token'] . $url, true);
				$data['shipping'] = $this->url->link('extension/purpletree_multivendor/sellerorders', 'user_token=' . $this->session->data['user_token'] . $url, true);
				$data['add'] = $this->url->link('extension/purpletree_multivendor/sellerorders', 'user_token=' . $this->session->data['user_token'] . $url, true);
				$data['delete'] = $this->url->link('extension/purpletree_multivendor/sellerorders', 'user_token=' . $this->session->data['user_token'] . $url, true);
				$data['sellerinvoice'] = $this->url->link('extension/purpletree_multivendor/sellerorders', 'user_token=' . $this->session->data['user_token'] . $url, true);
				} else {
				$data['invoice'] = $this->url->link('sale/order/invoice', 'user_token=' . $this->session->data['user_token'], true);
				$data['sellerinvoice'] = $this->url->link('extension/purpletree_multivendor/sellerorders/printsellerinvoice', 'user_token=' . $this->session->data['user_token'] . $url, true);
			...
			foreach ($results as $result) {
				...
				if(!$this->customer->validateSeller()) {
					$this->load->language('purpletree_multivendor/ptsmultivendor');
					$this->session->data['error_warning'] = $this->language->get('error_license');
					$edit = $this->url->link('extension/purpletree_multivendor/sellerorders', 'user_token=' . $this->session->data['user_token'] . $url, true);
					
					$view = $this->url->link('extension/purpletree_multivendor/sellerorders', 'user_token=' . $this->session->data['user_token'] . $url, true);
					}else {
			...
			$total_paid_amount = $this->model_extension_purpletree_multivendor_stores->getSellerPaidTotal($filter_data);
			if(!empty($results)){
		...
		public function printsellerinvoice() {
			...
			foreach ($orders as $order_id) {
				//
				$sellorders = array();
				$seller_order_info11 = $this->model_extension_purpletree_multivendor_orderinvoice->getsellerorder($order_id);
						...
						if(!empty($sellerorderArray)) {
							foreach($sellerorderArray as $sellerid => $sellerorders) {
								if($sellerid == 0) {
									...
									$this->load->model('extension/purpletree_multivendor/stores');
									$cus_seller_email = $this->model_extension_purpletree_multivendor_stores->getCustomerEmailId($sellerid);
...
public function download_pdi() {
				$this->load->model('sale/order');
				$product_designer = array();
				$canvas_images_d = $this->model_sale_order->getCanvasFromOrder($this->request->get['order_id'],$this->request->get['product_id']);
				require_once( DIR_SYSTEM.'library/purpletree_product_designer/fpdf.php' );

				if(!empty($canvas_images_d)) {
						if ($this->request->server['HTTPS']) {
					$image_full_d = HTTPS_CATALOG . 'image/';
				} else {
					$image_full_d = HTTP_CATALOG . 'image/';
				}
					$this->load->model('tool/image');
					$size_unit=0;
					foreach($canvas_images_d as $orderitems_d) {
						...
						list($width, $height, $type, $attr) = getimagesize($image_full_d.$orderitems_d['canvas_image']);
						$img_width[]=$width*$onepixel;
						$img_height[]=$height*$onepixel;
						$size_unit = $orderitems_d['size_unit'];
					}
					if($size_unit==1){
						$size_unit='mm';		
					} else if($size_unit==2){
				...
				$pdf = new FPDF('L',$size_unit,array($max_width,$max_height));
				
					foreach($canvas_images_d as $keyyy => $orderitems_d) {
						...
						$pdf->Image($image_full_d.$orderitems_d['canvas_image'],0,0,$img_width[$keyyy],$img_height[$keyyy]);
					}
					$pdf->Output("D","purpletree_product_designer.pdf");
				}
		
	}
				public function download_without_wm_pdf() {
				$this->load->model('sale/order');
				$product_designer = array();
				$canvas_images_d = $this->model_sale_order->getCanvasFromOrder($this->request->get['order_id'],$this->request->get['product_id']);
				require_once( DIR_SYSTEM.'library/purpletree_product_designer/fpdf.php' );
				...
				if(!empty($canvas_images_d)) {
					...
					foreach($canvas_images_d as $orderitems_d) {
						...
						list($width, $height, $type, $attr) = getimagesize($image_full_d.$orderitems_d['image_without_wm']);
						$img_width[]=$width*$onepixel;
						$img_height[]=$height*$onepixel;
						$size_unit = $orderitems_d['size_unit'];
					}
				...
				$pdf = new FPDF('L',$size_unit,array($max_width,$max_height));
					...
					foreach($canvas_images_d as $keyyy => $orderitems_d) {
						...
						$pdf->AddPage($orentation,$sizee);
						$pdf->Image($image_full_d.$orderitems_d['image_without_wm'],0,0,$img_width[$keyyy],$img_height[$keyyy]);
					}
					$pdf->Output("D","purpletree_product_designer.pdf");
				}
		...
		public function downloadjpeg() {
		
//purpletree_product_designer_status
				//$product['canvas_image_c'] = array();
				$image_url='';
				$pdfUrl='';
				$this->load->model('sale/order');
				$canvas_images = $this->model_sale_order->getCanvasFromOrder($this->request->get['order_id'],$this->request->get['product_id']);
				$productDesignStatus = $this->model_sale_order->getProductDesignStatus($this->request->get['product_id']);
				//if($productDesignStatus) {
				if($this->config->get('module_purpletree_product_designer_status')) {
				if(!empty($canvas_images)) {
				if ($this->request->server['HTTPS']) {
					$image_full = HTTPS_CATALOG . 'image/';
				} else {
					$image_full = HTTP_CATALOG . 'image/';
				}
					$this->load->model('tool/image');
					
					foreach($canvas_images as $key => $orderitems) {
						if($key == $this->request->get['key']) {
						$img=$image_full.$orderitems['image_without_wm'];	
					if(is_file(DIR_IMAGE.$orderitems['image_without_wm'])){						
						$createImg = imagecreatefrompng(DIR_IMAGE.$orderitems['image_without_wm']);
						$imgSize = GetImageSize($img);
						$newJpgFileName=pathinfo($orderitems['image_without_wm'], PATHINFO_FILENAME);
						$marge_right = 0;
						$marge_bottom = 0;
						if($imgSize[2] == 1){ 
							$img = imagecreatefromgif($img);
							$sx = imagesx($createImg);
							$sy = imagesy($createImg);
							$white = imagecolorallocate($img,  255, 255, 255);
							imagefilledrectangle($img, 0, 0, imagesx($createImg), imagesy($createImg), $white);
							imagecopy($img, $createImg, imagesx($img) - $sx - $marge_right, imagesy($img) - $sy - $marge_bottom, 0, 0, imagesx($createImg), imagesy($createImg));
							imagegif($img,DIR_IMAGE."catalog/ptsdesigner/".$newJpgFileName.'.jpg');
							$image_root="catalog/ptsdesigner/".$newJpgFileName.".jpg";
						}elseif($imgSize[2] == 2){
							...
							imagefilledrectangle($img, 0, 0, imagesx($createImg), imagesy($createImg), $white);
							imagecopy($img, $createImg, imagesx($img) - $sx - $marge_right, imagesy($img) - $sy - $marge_bottom, 0, 0, imagesx($createImg), imagesy($createImg));
							imagejpeg($img,DIR_IMAGE."catalog/ptsdesigner/".$newJpgFileName.'.jpg');
							$image_root="catalog/ptsdesigner/".$newJpgFileName.".jpg";
						}elseif($imgSize[2] == 3){
							...
							imagefilledrectangle($img, 0, 0, imagesx($createImg), imagesy($createImg), $white);
							imagecopy($img, $createImg, imagesx($img) - $sx - $marge_right, imagesy($img) - $sy - $marge_bottom, 0, 0, imagesx($createImg), imagesy($createImg));
							imagepng($img,DIR_IMAGE."catalog/ptsdesigner/".$newJpgFileName.'.jpg');
							$image_root="catalog/ptsdesigner/".$newJpgFileName.".jpg";
						}
						$image_url = $image_full.$image_root;
						}
						}
					}
				}
...
readfile($image_url); 
		...
		public function order_info(){
			...
			/// End Help code///
				...
				$data['sellerinvoice1'] = $this->url->link('extension/purpletree_multivendor/sellerorders/printsellerinvoice', 'user_token=' . $this->session->data['user_token']  . '&order_id=' . (int)$this->request->get['order_id'], true);
				//stripe payment
				$data['update_stripe_seller_payment']='';
				$stripe_status = $this->config->get('payment_pts_stripe_status');
				if($stripe_status){
					
				$product_seller = $this->model_extension_purpletree_multivendor_stores->updateIncompletePayment($this->request->get['order_id']);
				if(isset($product_seller) && count($product_seller) > 0){
				$data['update_stripe_seller_payment'] = $this->url->link('extension/purpletree_multivendor/sellerorders/update_stripe_payment', 'user_token=' . $this->session->data['user_token']  . '&order_id=' . (int)$this->request->get['order_id'], true);
					}
				}
				
				//stripe payment
				$data['curlpaypaypal'] = $this->url->link('extension/purpletree_multivendor/sellerorders/curlpaypaypal', 'user_token=' . $this->session->data['user_token']  . '&order_id=' . (int)$this->request->get['order_id'], true);
				...
				$product['canvas_image_c'] = array();
				$pdfUrl='';
				$canvas_images = $this->model_sale_order->getCanvasFromOrder($this->request->get['order_id'],$product['product_id']);
				$productDesignStatus = $this->model_sale_order->getProductDesignStatus($product['product_id']);
				//if($productDesignStatus) {
				if($this->config->get('module_purpletree_product_designer_status')) {
				if(!empty($canvas_images)) {
				if ($this->request->server['HTTPS']) {
					...
					$this->load->model('tool/image');
					foreach($canvas_images as $key => $orderitems) {
						$product['canvas_image_c'][$key] = array(
						'id' => $orderitems['id'],
						'image_label' => $orderitems['image_label'],
						'product_image'=> $this->model_tool_image->resize($orderitems['product_image'],20,20),
						'canvas_image'=> $this->model_tool_image->resize($orderitems['canvas_image'],20,20),
						'image_full'=> $image_full.$orderitems['product_image'],
						'canvas_image_full'=> $image_full.$orderitems['canvas_image'],
						'canvas_jpg_image_wtwm_full' => $this->url->link('extension/purpletree_multivendor/sellerorders/downloadjpeg', 'user_token=' . $this->session->data['user_token'] . '&product_id=' . $product['product_id']. '&key=' . $key. '&order_id=' . $this->request->get['order_id'], true)
						);
					}
				}
					$pdfUrl=$this->url->link('extension/purpletree_multivendor/sellerorders/download_pdi', 'user_token=' . $this->session->data['user_token'] . '&product_id=' . $product['product_id']. '&order_id=' . $this->request->get['order_id'], true);
					$pdfWithoutWMUrl=$this->url->link('extension/purpletree_multivendor/sellerorders/download_without_wm_pdf', 'user_token=' . $this->session->data['user_token'] . '&product_id=' . $product['product_id']. '&order_id=' . $this->request->get['order_id'], true);
			}
			//purpletree_product_designer_status
					$product_seller = $this->model_extension_purpletree_multivendor_stores->getStoreDetail($product['seller_id']);
					
					$data['products'][] = array(
					//purpletree_product_designer
					'canvas_images'  => $product['canvas_image_c'],
					//purpletree_product_designer
					...
					//purpletree_product_designer
					'img_download'     	 => isset($pdfUrl)?$pdfUrl:'',
				    'img_wm_download'     	 => isset($pdfWithoutWMUrl)?$pdfWithoutWMUrl:''
					//purpletree_product_designer
					);
				}
				...
				foreach ($totals as $total) {
					if($total['title'] == 'Refunded'){
						...
						$seller_idd = $ids[1];
						$sellerdd = $this->model_extension_purpletree_multivendor_stores->getsellerInfofororder($seller_idd); 
						$return_sellername  = $sellerdd['seller_name'];
						$return_storeurl = $this->url->link('extension/purpletree_multivendor/stores/edit&store_id='.$sellerdd['store_id'], 'user_token=' . $this->session->data['user_token'], true);
				...
				$data['refund_acton']=$this->url->link('extension/purpletree_multivendor/sellerorders/refundupdate', 'user_token=' . $this->session->data['user_token']. $url, true);
				...
				if(null !== $this->config->get('module_purpletree_multivendor_commission_status') && $order_info['order_status_id'] == $this->config->get('module_purpletree_multivendor_commission_status') && $order_info['payment_code'] == 'pp_adaptive') {
					...
					$getpayoutss = $this->getpayoutInformation($payout_batch_id,$order_id,$access_token);
						...
						if($this->config->get('payment_pp_adaptive_debug')){
							...
							foreach($getpayoutss as $keys=>$valuess){
								$invoiceidsss = $this->model_extension_payment_pp_adaptive->getpayoutcomm($order_id,$valuess['seller_id']);
								...
								$this->model_extension_purpletree_multivendor_commissioninvoice->addCommissionHistory($data);
	...
	public function update_stripe_payment() {
				$order_id=0;
			if(isset($this->request->get['order_id'])){
				$order_id=$this->request->get['order_id'];
				...
				$this->load->model('extension/purpletree_multivendor/stores');
				$transaction_ids= $this->model_extension_purpletree_multivendor_stores->updateIncompletePayment($order_id);
					 $messageseller='';
				$curency_code = $this
				->model_extension_purpletree_multivendor_stores->getCurrenyCode($order_id);
$this->log->write($transaction_ids);
				if(!empty($transaction_ids)){
					foreach($transaction_ids as $data){
						
						$seller_data= $this->model_extension_purpletree_multivendor_stores->getDataByInvoiceId($data['invoice_id']);
							 $accountId = $this->model_extension_purpletree_multivendor_stores->getStripeAccountID($seller_data['seller_id']);
						$payment_status= $this->model_extension_purpletree_multivendor_stores->getPaymentStatus($data['invoice_id'],$seller_data['seller_id']);
				//Update stripe status	
			if($payment_status!='Complete'){				
					try {
						$this->log->write('try2');
						if($data['txn_id'] == '') {
							 $pay_amount= number_format((float)$seller_data['pay_amount'], 2, '.', '')*100;
			if ($this->config->get('payment_pts_stripe_debug')) {
			...
			$messageseller.=$e->getMessage(); 
				...
				$this->model_extension_purpletree_multivendor_stores->saveTranDetail($transData);
				$this->model_extension_purpletree_multivendor_stores->saveTranHistory($transData); 
						} else {
					$payout= \Stripe\Payout::retrieve($data['txn_id']);
					...
					if($payout['status']=='paid'){
							...
							$status='Complete';
							$status_id=2;
							$messageseller='Stripe payment completed successfully.';
						...
						$this->model_extension_purpletree_multivendor_stores->updateTranDetail($transData);
						$this->model_extension_purpletree_multivendor_stores->insertPaymentHistory($transData);
						}
					}
				...
				//payout to seller
				if(isset($payout['status'])){
				if($payout['status']!='paid' || $payout['status']=='pending' || $payout['status']=='in_transit'){
				$msgseller='';
				 $accountId = $this->model_extension_purpletree_multivendor_stores->getStripeAccountID($seller_data['seller_id']);
				 $invoiceData = $this->model_extension_purpletree_multivendor_stores->getInvoieData($data['invoice_id']);	
				 $pay_amount= number_format((float)$invoiceData['total_pay_amount'], 2, '.', '')*100;
			 	try {
					$reCreatePayout = \Stripe\Payout::create([
					  'amount' => $pay_amount,
					  'currency' => $curency_code,
					], [
					  'stripe_account' => $accountId,
					]);	 
				}
					...
					$msgseller.=$e->getMessage(); 
				} 
				
				$status = 'Pending';
				$status_id=1;
				$msg="Seller Payment is pending";
				if(isset($reCreatePayout['status'])){
				 if($reCreatePayout['status']=='paid'){
					$status_id=2;
					$status='Complete';
					$msg="Seller Payment is Completed";
					} 
				} 
				$payoutId='';
				if(isset($reCreatePayout['id'])){
					$payoutId=$reCreatePayout['id'];
				}
				$msgseller.=$msg;
				 $transData=array(
							'invoice_id'		=> $data['invoice_id'],
			...
			//Resend payout to seller
		}
	}
$url ='';
				$this->response->redirect($this->url->link('extension/purpletree_multivendor/sellerorders/order_info', 'user_token=' . $this->session->data['user_token'] . '&order_id=' . $this->request->get['order_id'] . $url, true));
				} else {
				$this->response->redirect($this->url->link('extension/purpletree_multivendor/sellerorders', 'user_token=' . $this->session->data['user_token'] . $url, true));
				}
			} else {
				$this->response->redirect($this->url->link('extension/purpletree_multivendor/sellerorders', 'user_token=' . $this->session->data['user_token'] . $url, true));
			}
		...
		public function refundupdate() {
			if (isset($this->request->post['code'])) {
				$this->load->model('extension/purpletree_multivendor/stores');
				$this->model_extension_purpletree_multivendor_stores->updateRefund($this->request->post);
			}
			$url = '';
			if(isset($this->request->post['refund_order_id'])){
				$order_id = $this->request->post['refund_order_id'];
				$this->response->redirect($this->url->link('extension/purpletree_multivendor/sellerorders/order_info', 'user_token=' . $this->session->data['user_token'] . '&order_id=' . $order_id . $url, true));
				}else{
				$this->response->redirect($this->url->link('extension/purpletree_multivendor/sellerorders', 'user_token=' . $this->session->data['user_token'] . $url, true));
			}        
		}
