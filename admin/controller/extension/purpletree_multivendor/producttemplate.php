		...
		public function delete() {
			...
			if (isset($this->request->post['selected']) && $this->validateDelete()) {
				foreach ($this->request->post['selected'] as $id) {	
				   $template_id = $this->model_extension_purpletree_multivendor_vendor->getTemplateId1($id);
					$product_id = $this->model_extension_purpletree_multivendor_vendor->getProductid($template_id);
					$temp_status = $this->model_extension_purpletree_multivendor_vendor->checkTempstatus($product_id);
					$status1 = '';			
					if($temp_status == 1){
						   $status1 = 1;
...
// Options
		...
		if (isset($this->request->get['id'])) {
			...
			$product_options = $this->model_extension_purpletree_multivendor_vendor->getProductOptions($this->request->get['id']);
...
// die;
		...
		$data['option_values'] = array();
			...
			if ($product_option['type'] == 'select' || $product_option['type'] == 'radio' || $product_option['type'] == 'checkbox' || $product_option['type'] == 'image') {
			if (!isset($data['option_values'][$product_option['option_id']])) {
			$data['option_values'][$product_option['option_id']] = $this->model_extension_purpletree_multivendor_vendor->getOptionValuesByProductOptId($product_option['option_id'],$product_id);
			}
			}
			}
			...
			if (isset($this->request->get['id'])) {
				$data['action'] = $this->url->link('extension/purpletree_multivendor/producttemplate/editTemplate', 'user_token=' . $this->session->data['user_token'] . '&product_id='.$data['product_id'].'&id=' . $this->request->get['id'] . $url, true);
			}
			$data['stock_statuses'] = $this->model_localisation_stock_status->getStockStatuses();
		...
		public function editTemplate() {
			...
			$tproduct_id= $this->model_extension_purpletree_multivendor_vendor->getProductid($this->request->post['template_id']);
		...
		protected function validateFormTemp() {
			...
			$template_num_rows = $this->model_extension_purpletree_multivendor_vendor->getEditTemplateData($this->request->post['template_id'],$this->request->post['seller_id_filter'],$this->request->post['id']); 
	...
	public function getTemplateProductOptions() {
		...
		if (isset($this->request->get['filter_name']) && isset($this->request->get['product_id'])) {
			...
			foreach ($options as $option) {
				$results = $this->model_extension_purpletree_multivendor_vendor->getTemplateProductStatus($product_id,$option['option_id']);
				...
				$checkOptions = $this->model_extension_purpletree_multivendor_vendor->getTemplateProductOption($data);
				if($checkOptions){
