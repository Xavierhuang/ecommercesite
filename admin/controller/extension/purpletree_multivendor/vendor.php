...
class ControllerExtensionPurpletreeMultivendorVendor extends Controller{
		...
		public function deleteSeller(){
			...
			if ($this->request->server['REQUEST_METHOD'] == 'POST') {
				...
				// Delete products
				if($delete_product == 1) {
					$sellerProducts = $this->model_extension_purpletree_multivendor_vendor->getsellerProducts($seller_id);
				...
				$this->model_extension_purpletree_multivendor_vendor->deleteAllinfoSeller($seller_id);
		...
		public function confirmdeleteSeller(){
			...
			$this->load->model('extension/purpletree_multivendor/vendor');
			$results = $this->model_extension_purpletree_multivendor_vendor->getVendors();
			$data['delete'] = $this->url->link('extension/purpletree_multivendor/vendor/deleteSeller', 'user_token=' . $this->session->data['user_token'] . '&seller_id='.$this->request->get['seller_id'] . $url, true);
			///Help code///
		...
		protected function getList() {
			...
			'href' => $this->url->link('customer/customer/add', 'user_token=' . $this->session->data['user_token'] . $url, true)
			);
			
			//$data['add'] = $this->url->link('extension/purpletree_multivendor/vendor/add', 'user_token=' . $this->session->data['user_token'] . $url, true);
			...
			foreach ($results as $result) {
				if (!$this->customer->validateSeller()) {
					...
					$edit = $this->url->link('extension/purpletree_multivendor/vendor', 'user_token=' . $this->session->data['user_token'], true);
					} else {
					$send_message = $this->url->link('extension/purpletree_multivendor/sellerenquiries/sendsellermessage', 'user_token=' . $this->session->data['user_token'] . '&seller_id=' . $result['seller_id'] . $url, true);
					$delete=$this->url->link('extension/purpletree_multivendor/vendor/confirmdeleteseller', 'user_token=' . $this->session->data['user_token'] . '&seller_id=' . $result['seller_id'] . $url, true);
					$edit = $this->url->link('customer/customer/edit', 'user_token=' . $this->session->data['user_token'] . '&customer_id=' . $result['seller_id'] . $url, true);
				}
				if($this->config->get('module_purpletree_multivendor_subscription_plans')) {
					$hasPlan = $this->model_extension_purpletree_multivendor_vendor->getSubscriptionplan($result['seller_id']);
					$plan = $this->url->link('extension/purpletree_multivendor/managesubscriptionplan/add&user_token=' . $this->session->data['user_token'] . '&seller_id='. $result['seller_id'].'&action=add'  , true);
					$plan1 = $this->url->link('extension/purpletree_multivendor/managesubscriptionplan/viewallplan&user_token=' . $this->session->data['user_token'] . '&seller_id='. $result['seller_id'] , true);
