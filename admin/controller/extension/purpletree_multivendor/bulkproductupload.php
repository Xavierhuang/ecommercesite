...
class ControllerExtensionPurpletreeMultivendorBulkproductupload extends Controller {
		...
		public function export(){
			...
			if ($category_info) {
			$catPath= str_replace("&nbsp;", "",$category_info['path']);
			$cate[] = ($catPath) ? $catPath . ' &gt; ' . $category_info['name'] : $category_info['name'];
			...
			{
			if($key!=1) {	
			// Pre-import validation: clear error reporting for this row
			$row_errors = array();
			...
			if($sourcecode !== FALSE) {
			if(!empty($this->request->post['seller_id'])){
			$imagesourc = 'catalog/Seller_'.$this->request->post['seller_id'].'/'.$image_prefix.basename(trim($data['image']));
			}else{
			$imagesourc = 'catalog/'.$image_prefix.basename(trim($data['image']));
			}							
			}
			...
			if($sourcecode !== FALSE){							
			if(!empty($this->request->post['seller_id'])){
			$savefile = @fopen(DIR_IMAGE."catalog/Seller_".$this->request->post['seller_id']."/".$image_prefix.basename(trim($data['image'])), 'w');
			}else{
			$savefile = @fopen(DIR_IMAGE."catalog/".$image_prefix.basename(trim($data['image'])), 'w');
			}
			...
			$product_id=$this->model_extension_purpletree_multivendor_bulkproductupload->checkExistingProductIds($commonData['column_name'],$filter_data[$commonData['column_name']]);
				...
				$exp= explode(',',preg_replace('/^\p{Z}+|\p{Z}+$/u', '', $value['category']));
					...
					foreach($exp as $vvv){
						...
						if(!empty($dddd)) {
							...
							//When 3rd child category found in sheet
							$firstids = $this->model_extension_purpletree_multivendor_bulkproductupload->getCategoryIds(htmlspecialchars(trim($thirdast),ENT_QUOTES),$this->request->post['language']);
							if(!empty($firstids)) {
							foreach($firstids as $firstid) {
								$topcatg = $firstid['category_id'];
								foreach($secarray as $slast => $last) {
			...
			if($key!=1){
				/* echo "<Pre>";
				print_r($valueDiscount['product_id']);
				print_r($filterProData);
				print_r($commonData['column_name']);
				//print_r($filterProData[$valueDiscount['product_id']][$commonData['column_name']]);
				die; */
			if($commonData['action']=='update'){	
			...
			if(!empty($this->request->post['seller_id'])){
			$image_path = 'catalog/Seller_'.$this->request->post['seller_id'].'/'.$image_prefix.basename(trim($valueImage['image']));
			}else{
			$image_path = 'catalog/'.$image_prefix.basename(trim($valueImage['image']));
			}
			if($sourcecode !== FALSE) {
			...
			if(!empty($this->request->post['seller_id'])){				 
			$savefile = @fopen(DIR_IMAGE."catalog/Seller_".$this->request->post['seller_id']."/".$image_prefix.basename(trim($valueImage['image'])), 'w');
			}else{
			$savefile = @fopen(DIR_IMAGE."catalog/".$image_prefix.basename(trim($valueImage['image'])), 'w');
			}
			@fwrite($savefile, $sourcecode);
			@fclose($savefile);	
	...
	public function getProductData($productDataSheet){
		...
		return $filterProData;
			...
			$data['add_product'] = $this->url->link('extension/purpletree_multivendor/bulkproductupload/addproduct', 'user_token=' . $this->session->data['user_token'], true);
			$data['update_product'] = $this->url->link('extension/purpletree_multivendor/bulkproductupload/updateproduct', 'user_token=' . $this->session->data['user_token'], true);
