...
class ControllerExtensionPurpletreeMultivendorSellercommission extends Controller{
		...
		public function index(){
			...
			$data['results'] = '';
			if(isset($this->session->data['generated_invoice_order_id'])){
			$data['generated_invoice_order_id'] = $this->session->data['generated_invoice_order_id'];
			}	
		...
		public function generate(){
			...
			foreach($this->request->post['selected'] as $key => $value){
			 $commission_invoice_check = $this->model_extension_purpletree_multivendor_sellercommission->checkCommissionInvoice($value,$this->request->get['seller_id']);
			 if($commission_invoice_check == 1){
			 $generated_invoice_order_id =  $value;
			 }
			}
			}
			if($commission_invoice_check != 1){
		  ...
		  $this->session->data['error_warning'] = $this->language->get('error_alrady_generated');
		  unset($this->session->data['generated_invoice_order_id']);
		  $this->session->data['generated_invoice_order_id'] = $generated_invoice_order_id;
		}
			$seller_id ='';
			if (isset($this->request->get['seller_id'])) {
