<?php
class ControllerExtensionCommonFileManager extends Controller {
		public function index() { 
			$this->load->language('common/filemanager');
			$this->document->addStylepts('catalog/view/theme/default/stylesheet/purpletree/custom.css');
			...
			foreach ($images as $image) {
				...
				if (is_dir($image)) {
					...
					/* if (isset($this->request->get['seller_name'])) {
						$url .= '&seller_name=' . str_replace(" ","_",$this->request->get['seller_name']);
					} */
					...
					'href'  => $this->url->link('extension/common/filemanager', 'directory=' . urlencode(utf8_substr($image, utf8_strlen(DIR_IMAGE . 'catalog/'))) . $url, true)
					);
			...
			$data['parent'] = $this->url->link('extension/common/filemanager', $url, true);
			...
			$data['refresh'] = $this->url->link('extension/common/filemanager',$url, true);
			...
			$pagination->limit = 16;
			$pagination->url = $this->url->link('extension/common/filemanager', $url . '&page={page}', true);
			
			$data['pagination'] = $pagination->render();
			
			$this->response->setOutput($this->load->view('common/pts_filemanager', $data));
		}
