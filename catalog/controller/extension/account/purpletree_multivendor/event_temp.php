...
class ControllerExtensionAccountPurpletreeMultivendorEvents extends Controller {
		...
		if(!empty($output)){
			foreach($output as $pKey => $pVal){
			$minPriceData = $this->load->controller('product/product/getTemplateData',$pKey);
				if(!empty($minPriceData)){
					$output[$pKey]['price']=$minPriceData['price'];
					$output[$pKey]['quantity']=$minPriceData['quantity'];
					$output[$pKey]['stock_status_id']=$minPriceData['stock_status_id'];
					$output[$pKey]['subtract']=$minPriceData['subtract'];
					$output[$pKey]['status']=$minPriceData['status'];
				}
			}
		}
		//template product filter
