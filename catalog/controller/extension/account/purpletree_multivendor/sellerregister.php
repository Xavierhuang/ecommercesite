...
class ControllerExtensionAccountPurpletreeMultivendorSellerregister extends Controller {
		...
		public function callback(){
            if(isset($this->session->data['seller_session']) && !empty($this->session->data['seller_session']) ){
                
                $this->request->post = $this->session->data['seller_session']['post'] ;
                $this->request->get = $this->session->data['seller_session']['get'];
                $this->request->files = $this->session->data['seller_session']['files'];
                $this->request->server = $this->session->data['seller_session']['server'];
                $this->request->cookie = $this->session->data['seller_session']['cookie'];
                unset($this->request->post['membership']);
                unset($this->session->data['seller_session']);
                $this->index();
            }
        }
		public function index() {
			...
			if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			 if(isset($this->request->post['membership'])){
                 $membership_info = $this->model_extension_purpletree_multivendor_sellers->getSellerPackage($this->request->post['membership']);
                 //print_r($membership_info);exit;
                 if($membership_info && $membership_info['recurring_id'] != 2){
                     //print_r($membership_info);exit;
                     $pay_lib_root=DIR_SYSTEM.'library/aw_stripe/vendor/';
                     ...
                     if($payment_mode>0){
                         $stripe = new \Stripe\StripeClient($this->config->get('payment_pts_stripe_secret_key_live'));
                     }else{
                         $stripe = new \Stripe\StripeClient($this->config->get('payment_pts_stripe_secret_key_test'));
                     }
                     try{
                         $membership_product = $stripe->products->create([
                     ...
                     //print_r($membership_product);exit;
                     if($membership_info['type']==1 && $membership_info['recurring_id']>0){
                         //Fixed Price
                         try{
                         $membership_price = $stripe->prices->create([
                           'unit_amount' => (int)($membership_info['amount']*100),
                           'currency' => 'usd',
                           'product' => $membership_product->id,
                         ]);
                         } catch (Stripe_InvalidRequestError $e) {
                             print_r($e);
                         // Invalid parameters were supplied to Stripe's API
                         ...
                         $plan_detail=$this->model_extension_purpletree_multivendor_subscriptionplan->getPlanDetail($membership_info['recurring_id']);
                         
                         //var_dump($plan_detail);exit;
                         
                         try{
                             $validity = $plan_detail['validity']/(12*30);
                         $membership_price = $stripe->prices->create([
                           'unit_amount' => (int)($plan_detail['subscription_price']*100),
                           'currency' => 'usd',
                           'recurring' => ['interval' => ($validity>1)?'year':'month'],
                           'product' => $membership_product->id,
                         ]);
                             
                         /*$membership_price_yearly =   $stripe->prices->create([
                          'unit_amount' => (int)($plan_detail['subscription_price']*100*12),
                          'currency' => 'usd',
                     ...
                     $link_response = $stripe->paymentLinks->create([
                        'line_items' => [
                            ...
                            /*[
                              'price' => $membership_price_yearly->id,
                              'quantity' => 1,
                            ],*/
                          ],
                          'after_completion' => [
                            'type' => 'redirect',
                            'redirect' => ['url' => $this->url->link('extension/account/purpletree_multivendor/sellerregister/callback', '', true)],
                          ],
                     ]);
                     } catch (Stripe_InvalidRequestError $e) {
                         print_r($e);
                     ...
                     if($link_response['url']){
                         $this->session->data['seller_session']['post'] = $this->request->post;
                         $this->session->data['seller_session']['get'] = $this->request->get;
                         $this->session->data['seller_session']['files'] = $this->request->files;
                         $this->session->data['seller_session']['server'] = $this->request->server;
                         $this->session->data['seller_session']['cookie'] = $this->request->cookie;
                         $this->session->data['seller_session']['post']['membership_commission'] = $membership_info['commission'];
                         $this->session->data['seller_session']['post']['membership_plan_id'] = $membership_info['recurring_id'];
                         $this->response->redirect($link_response['url']);
                     }else{
                         print_r($link_response);
                         print_r($membership_price);exit;
                         $this->log->write('Link Response: ' .json_encode($link_response));
                     }
                 }elseif($membership_info){
                     if($membership_info['amount']==0){
                         $this->request->post['membership_commission'] = $membership_info['commission'];
                         $this->request->post['membership_plan_id'] = $membership_info['recurring_id'];
                     }
                }else{
                     $this->session->data['error_warning'] = "Invalid Membership";
                     $this->response->redirect($this->url->link('extension/account/purpletree_multivendor/sellerregister', '', true));
                 }
                 //$this->response->redirect($this->request->post['membership']);
                 //exit;
             }
                $member_ship_plan_id = isset($this->request->post['membership_plan_id'])?$this->request->post['membership_plan_id']:0;
                //$this->log->write('Membership Plan : ' .$this->request->post['membership_plan_id']);
                //$this->log->write('SUCCESS POST: ' .json_encode($this->request->post));
                //seller area //
		    ...
		    //seller area //
				...
				if ($customer_id){
					$this->subscribePlan($customer_id,$member_ship_plan_id);
				}
				...
				if (isset($customer_group_info['approval'])) {
			...
			if ($this->config->get('module_purpletree_multivendor_seller_ac_terms')!=null) {
				$this->load->model('catalog/information');
				
				$information_info = $this->model_catalog_information->getInformation($this->config->get('module_purpletree_multivendor_seller_ac_terms'));
				
				if ($information_info) {
					$data['text_agree'] = sprintf($this->language->get('text_agree'), $this->url->link('information/information/agree', 'information_id=' . $this->config->get('module_purpletree_multivendor_seller_ac_terms'), true), $information_info['title'], $information_info['title']);
					} 
				} else {
            ...
            //Membership Packages
            $data['membership_packages'] = $this->model_extension_purpletree_multivendor_sellers->getSellerPackages();
			...
			$data['hide_fields']=$this->config->get('module_purpletree_multivendor_hide_reg');
			...
			$this->document->addStylepts('catalog/view/javascript/purpletree/css/stylesheet/commonstylesheet.css');
		...
		private function validate() {
			...
			if(!empty($custom_fields)) {
				foreach ($custom_fields as $custom_field) {
					if (($custom_field['location'] == 'account') && ($custom_field['step_location'] != 'one')) {
						if ($custom_field['required'] && empty($this->request->post['custom_field'][$custom_field['location']][$custom_field['custom_field_id']])) {
							$this->error['custom_field'][$custom_field['custom_field_id']] = sprintf($this->language->get('error_custom_field'), $custom_field['name']);
						} elseif (($custom_field['type'] == 'text') && !empty($custom_field['validation']) && !filter_var($this->request->post['custom_field'][$custom_field['location']][$custom_field['custom_field_id']], FILTER_VALIDATE_REGEXP, array('options' => array('regexp' => $custom_field['validation'])))) {
							$this->error['custom_field'][$custom_field['custom_field_id']] = sprintf($this->language->get('error_seller_validation'), $custom_field['name']);
						}
					}
				}
			}
			...
			if (!$this->customer->isLogged()) {
				...
				// seller Custom field validation
		...
		// End hyper local
		public function subscribePlan($customer_id,$member_ship_plan_id=0) {
			$url="";							
			...
			if($plan_id){	
				...
				$startt_when =0;
				$s_date = 0;
				$data['plan_id']=($member_ship_plan_id>0)?$member_ship_plan_id:$plan_id;
				...
				'title'=>$this->language->get('column_subscription_price'),
				...
				if($this->config->get('module_purpletree_multivendor_multiple_subscription_plan_active')){		
					if($startt_when!=1){			
						...
						'sort_order'=>1,
						'code'=>'joining_fee',
						'title'=>$this->language->get('text_joining_fee'),
						'value'=>$joining_fee
						);
						} else {
						$joining_fee=0;
				...
				if(!$this->config->get('module_purpletree_multivendor_multiple_subscription_plan_active')){
					if(isset($currentplan)){				
						...
						'title'=>$this->language->get('text_adjustment_joiningfee'),
						'value'=>$a_joiningfee
						);	
						...
						'title'=>$this->language->get('text_adjustment_price'),
						'value'=>$subscription_price
						);
						$previous_balance=0;
				...
				if(!$this->config->get('module_purpletree_multivendor_multiple_subscription_plan_active')){
					...
					'title'=>$this->language->get('text_previous_balance'),
					'value'=>$invoice_bal
					);
				}
				$total=$total_amount+$cal_tax+$current_invo;
				...
				$plan_result=$this->model_extension_purpletree_multivendor_subscriptionplan->getSubscribePlanDetail($plan_id,$invoice_id);
				
				$start=($plan_result['start_date']!='0000-00-00 00:00:00')?date('d/m/Y H:i:s',strtotime($plan_result['start_date'])):'';
				$end=($plan_result['end_date']!='0000-00-00 00:00:00')?date('d/m/Y H:i:s',strtotime($plan_result['end_date'])):date('d/m/Y H:i:s', strtotime($plan_result['start_date']. ' + '.$plan_result['validity'].' days'));
				...
				//end
				 if(!$sellerExist){
					$sellerExist=$this->model_extension_purpletree_multivendor_subscriptionplan->addFirstSellerPlan($customer_id);
                     if($member_ship_plan_id>0){
                         $this->model_extension_purpletree_multivendor_subscriptionplan->enableSellerSubscription($customer_id);
                         $invoice_id=$this->model_extension_purpletree_multivendor_subscriptionplan->addSellerPlan($data);
                     }
				   ...
				   $messtemplatefromdb = $register_template['new_message'];			
					...
					$replacevar = array('_SELLER_NAME_' =>$seller_name,
												...
												'_GRAND_TOTAL_' =>$this->currency->format($total, $this->session->data['currency'])
												);
		...
		public function checkseller(){
			$this->load->model('extension/purpletree_multivendor/sellers');
			$json['status'] = $this->model_extension_purpletree_multivendor_sellers->getSellersFilter($this->request->get['seller_area']);
			$this->response->addHeader('Content-Type: application/json');
			$this->response->setOutput(json_encode($json));
