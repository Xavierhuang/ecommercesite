...
class ControllerExtensionAccountPurpletreeMultivendorBulkproductupload extends Controller{
		...
		public function export(){
			...
			if(!$this->customer->validateSeller()) {
				...
				$this->response->redirect($this->url->link('extension/account/purpletree_multivendor/bulkproductupload', '', true));
			}
		...
		public function exportExcelData($objPHPexl,$tabName,$tblName,$seller_id,$language,$sheetIndex){
			...
			if($tblName=='product_description')
			{
				...
				$tableName=array('A'=>'product_id','B'=>'name','C'=>'description','D'=>'tag','E'=>'meta_title','F'=>'meta_description','G'=>'meta_keyword');
			}
			if($tblName=='product')
			{
		...
		public function exportLinksData($objPHPexl,$tabName,$seller_id,$language,$sheetIndex){	
			...
			$download = array_search("download_id",$dd);
			if($download){
				$dd[$download]='download';
			}
			
			$category = array_search("category_id",$dd);
			if($category){
				$dd[$category]='category';
			}
			
			$filter = array_search("filter_id",$dd);
			if($filter){
				$dd[$filter]='filter';
			}
			$related = array_search("related_id",$dd);
			if($related){
				$dd[$related]='related';
			}
			$store = array_search("store_id",$dd);
			if($store){
				$dd[$store]='store';
			}
							...
							if(!empty($fetchData[$value_product_id])) {
								...
								if(!empty($categories)){
									foreach($categories as $categoryid){
										$category_info = $this->model_extension_purpletree_multivendor_bulkproductupload->getCategory($categoryid,$this->request->post['language']);
							...
							if(!empty($fetchData[$value_product_id])) {
								...
								if(!empty($filterexp)){
									foreach($filterexp as $filterId){
										$filter_info = $this->model_extension_purpletree_multivendor_bulkproductupload->getFilter($filterId);
							...
							if(!empty($fetchData[$value_product_id])) {
								if(!empty($productrelated['related_id'])){
									...
									foreach($relatedProduct as $relatedP){
										$related_info = $this->model_extension_purpletree_multivendor_bulkproductupload->getProduct($relatedP,$this->request->post['language']);
		...
		/* ************************************************************************************ */
			...
			if($this->config->get('module_purpletree_multivendor_subscription_plans')){
				...
				if($this->config->get('module_purpletree_multivendor_multiple_subscription_plan_active')){
					$total_plan_Product = $this->model_extension_purpletree_multivendor_bulkproductupload->getNoOfProductForMultiplePlan($this->customer->getId());
					} else {
					$total_plan_Product = $this->model_extension_purpletree_multivendor_bulkproductupload->getNoOfProduct($this->customer->getId());
				}
				$plan_product=0;
			if(!empty($total_plan_Product['no_of_product'])){
				...
				$getSsellerplanStatus = $this->model_extension_purpletree_multivendor_bulkproductupload->getSsellerplanStatus($this->customer->getId());
			...
			if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
				...
				$sseller_id = $this->customer->getId();
										...
										if($key!=1) {	
											...
											try {
												if(is_numeric($data['product_id']) && $data['product_id']!='' && !is_string($data['product_id']))
												{
													...
													$checkSellerVacation = $this->model_extension_purpletree_multivendor_sellerproduct->checkSellerVacationProduct($this->customer->getId());
													if($checkSellerVacation>=1){	
														$data_status=0;
													}
													$imagesourc = '';
													if($sourcecode !== FALSE) {							
														$imagesourc = 'catalog/Seller_'.$sseller_id.'/'.$image_prefix.basename(trim($data['image']));
													}
													...
													if($addProduct_action==='YES' ){
														...
														if($this->config->get('module_purpletree_multivendor_subscription_plans')){
															...
															$total_store_Product = $this->model_extension_purpletree_multivendor_bulkproductupload->sellerTotalProduct($this->customer->getId());
														...
														$check_existing_product=$this->model_extension_purpletree_multivendor_bulkproductupload->checkExistingProduct($commonData['column_name'],$filter_data[$commonData['column_name']]);
														...
														if($sourcecode !== FALSE){	
																$sseller_id = $this->customer->getId();
																$savefile = @fopen(DIR_IMAGE."catalog/Seller_".$sseller_id."/".$image_prefix.basename(trim($data['image'])), 'w');
														...
														{
															if(isset($filter_data['product_id']) && $this->customer->getId()){
																if(!empty($commonData['column_name'])){
																	
																$product_id=$this->model_extension_purpletree_multivendor_bulkproductupload->getExistingProductId($commonData['column_name'],$filter_data[$commonData['column_name']],$this->customer->getId());
																//$product_check=$this->model_extension_purpletree_multivendor_bulkproductupload->getInProductId($filter_data['product_id'],$this->customer->getId());
									...
									foreach($ddd as $key11 => $ffff) {
										...
										if($key11 != 1){
											if($commonData['action']=='update'){
												$ffff['product_id']=$this->model_extension_purpletree_multivendor_bulkproductupload->checkExistingProductIds($commonData['column_name'],$filterProData[$ffff['product_id']][$commonData['column_name']]);	
											}
							...
							try {
								...
								if(!empty($excelSheetLinkData) && $linkvalidate && $gen && $produc ) {
									foreach($excelSheetLinkData as $key=>$value)
									{
										...
										if($key!=1){
												if($commonData['action']=='update'){
												$value['product_id']=$this->model_extension_purpletree_multivendor_bulkproductupload->checkExistingProductIds($commonData['column_name'],$filterProData[$value['product_id']][$commonData['column_name']]);	
											}
				...
				$exp= explode(',',preg_replace('/^\p{Z}+|\p{Z}+$/u', '', $value['category']));
					...
					foreach($exp as $vvv){
						...
						if(!empty($impData)){
							...
							$lastids		= $this->model_extension_purpletree_multivendor_bulkproductupload->getCategoryIds(htmlspecialchars(preg_replace('/^\p{Z}+|\p{Z}+$/u', '', $lastname),ENT_QUOTES),$this->request->post['language']);
							$dddd = array();
							if(!empty($lastids)) {
								foreach($lastids as $lastiddd) {
									
									$x = $this->model_extension_purpletree_multivendor_bulkproductupload->getParentId($lastiddd['category_id']);
									if(!empty($x)) {
						...
						if(!empty($dddd)) {
							//
							$secarray = array();
							$selass		= $this->model_extension_purpletree_multivendor_bulkproductupload->getCategoryIds(htmlspecialchars(preg_replace('/^\p{Z}+|\p{Z}+$/u', '', $secondLast),ENT_QUOTES),$this->request->post['language']);
							if(!empty($selass)) {
							...
							$firstids = $this->model_extension_purpletree_multivendor_bulkproductupload->getCategoryIds(htmlspecialchars(preg_replace('/^\p{Z}+|\p{Z}+$/u', '', $thirdast),ENT_QUOTES),$this->request->post['language']);
							if(!empty($firstids)) {
							foreach($firstids as $firstid) {
								$topcatg = $firstid['category_id'];
								foreach($secarray as $slast => $last) {
									$x34 = $this->model_extension_purpletree_multivendor_bulkproductupload->getParentId($slast);
									if(!empty($x34)) {
							...
							try {
								...
								if(!empty($excelSheetAttributeData) && $attribvalidate && $gen && $produc ) {
									foreach($excelSheetAttributeData as $key=>$valueAttribute)
									{
										...
										if($key!=1){
											if($commonData['action']=='update'){
												$valueAttribute['product_id']=$this->model_extension_purpletree_multivendor_bulkproductupload->checkExistingProductIds($commonData['column_name'],$filterProData[$valueAttribute['product_id']][$commonData['column_name']]);	
											}
								...
								if(!empty($excelSheetRecurringData) && $recurrvalidate && $gen && $produc ) {
									foreach($excelSheetRecurringData as $key=>$valueRecurring)
									{
										...
										if($key!=1){
											...
											if($commonData['action']=='update'){
												$valueRecurring['product_id']=$this->model_extension_purpletree_multivendor_bulkproductupload->checkExistingProductIds($commonData['column_name'],$filterProData[$valueRecurring['product_id']][$commonData['column_name']]);	
											}
								...
								if(!empty($excelSheetDiscountData) && $discvalidate && $gen && $produc ) {
									foreach($excelSheetDiscountData as $key=>$valueDiscount)
									{
										...
										if($key!=1){
											...
											if($commonData['action']=='update'){
												$valueDiscount['product_id']=$this->model_extension_purpletree_multivendor_bulkproductupload->checkExistingProductIds($commonData['column_name'],$filterProData[$valueDiscount['product_id']][$commonData['column_name']]);	
											}
								...
								if (!empty($excelSheetSpecialData) && $specvalidate && $gen && $produc ) {
									foreach($excelSheetSpecialData as $key=>$valueSpecial)
									{
										...
										if($key!=1){
											...
											if($commonData['action']=='update'){
												$valueSpecial['product_id']=$this->model_extension_purpletree_multivendor_bulkproductupload->checkExistingProductIds($commonData['column_name'],$filterProData[$valueSpecial['product_id']][$commonData['column_name']]);	
											}
								...
								if (!empty($excelSheetRewardpointsData) && $rewardvalidate && $gen && $produc ) {
									foreach($excelSheetRewardpointsData as $key=>$valueRewardpoints)
									{
										...
										if($key!=1){
											...
											if($commonData['action']=='update'){
												$valueRewardpoints['product_id']=$this->model_extension_purpletree_multivendor_bulkproductupload->checkExistingProductIds($commonData['column_name'],$filterProData[$valueRewardpoints['product_id']][$commonData['column_name']]);	
											}
								...
								if(!empty($excelSheetSeoData) && $sevalidate && $gen && $produc ) {
									foreach($excelSheetSeoData as $key=>$valueSeo)
									{
										...
										if($key!=1){
											...
											if($commonData['action']=='update'){
												$valueSeo['product_id']=$this->model_extension_purpletree_multivendor_bulkproductupload->checkExistingProductIds($commonData['column_name'],$filterProData[$valueSeo['product_id']][$commonData['column_name']]);	
											}
								...
								if(!empty($excelSheetDesignData) && $desivalidate && $gen && $produc ) {
									foreach($excelSheetDesignData as $key=>$valueDesign)
									{
										...
										if($key!=1){
											...
											if($commonData['action']=='update'){
												$valueDesign['product_id']=$this->model_extension_purpletree_multivendor_bulkproductupload->checkExistingProductIds($commonData['column_name'],$filterProData[$valueDesign['product_id']][$commonData['column_name']]);	
											}
...
$status_msgg[] = array("status"=>"success", "msg" => "Product  ID ".$product_id." Design Data Uploaded Successfully");
						...
						if($this->customer->getId() && $gen && $produc ) {
								...
								try {
									...
									if(!empty($excelSheetSellerData)) {
										foreach($excelSheetSellerData as $key=>$valueseller)
										{
											...
											if($commonData['action']=='update'){
												$valueseller['product_id']=$this->model_extension_purpletree_multivendor_bulkproductupload->checkExistingProductIds($commonData['column_name'],$filterProData[$valueseller['product_id']][$commonData['column_name']]);	
											}
												...
												if(!in_array($valueseller['product_id'],$failed_array)) {
													...
													if(in_array($product_id,$passed_array)) {
														...
														'product_id'	=>		$product_id,
														'is_approved'	=>		($this->config->get('module_purpletree_multivendor_product_approval'))?'0':'1'
														);
														try {
															if(is_numeric($product_id) && $product_id !='') {
																if (array_key_exists($product_id_old,$new_products)) {
								...
								if(!empty($excelSheetImageData) && $imgvalidate && $gen && $produc ) {
									foreach($excelSheetImageData as $key=>$valueImage)
									{
										...
										if($key!=1){
											...
											if($commonData['action']=='update'){
												$valueImage['product_id']=$this->model_extension_purpletree_multivendor_bulkproductupload->checkExistingProductIds($commonData['column_name'],$filterProData[$valueImage['product_id']][$commonData['column_name']]);	
											}
											...
											if(!in_array($valueImage['product_id'],$failed_array)) {
												...
												if(in_array($product_id,$passed_array)) {	
													...
													if($sourcecode !== FALSE) {
														...
														'product_image_id'			=>		$valueImage['product_image_id'],
														'product_id'				=>		$product_id,
														'image'						=>		'catalog/Seller_'.$sseller_id.'/'.$image_prefix.basename(trim($valueImage['image'])),
															...
															if(is_numeric($product_id) && $product_id !='') {
																if (array_key_exists($product_id_old,$new_products)) {
																	...
																	if($product_image_id ==''){
																		...
																		$savefile = @fopen(DIR_IMAGE."catalog/Seller_".$sseller_id."/".$image_prefix.basename(trim($valueImage['image'])), 'w');
																		@fwrite($savefile, $sourcecode);
																		@fclose($savefile);	
						...
						if(!empty($excelSheetProductOptionData) && $prooptvalidate && $gen && $produc ) {
							foreach($excelSheetProductOptionData as $key=>$valueOption)
							{
								...
								if($key!=1){
											...
											if($commonData['action']=='update'){
												$valueOption['product_id']=$this->model_extension_purpletree_multivendor_bulkproductupload->checkExistingProductIds($commonData['column_name'],$filterProData[$valueOption['product_id']][$commonData['column_name']]);	
											}
									...
									if(!in_array($valueOption['product_id'],$failed_array)) {
										...
										if(in_array($product_id,$passed_array)) {
											...
											$opt_id=$this->model_extension_purpletree_multivendor_bulkproductupload->getOptionId($valueOption['name']);
						...
						if(!empty($excelSheetProductOptionValueData) && $prooptvaluevalidate && $gen && $produc ) {
							foreach($excelSheetProductOptionValueData as $key=>$valueProductOption)
							{
								...
								if($key!=1){
											...
											if($commonData['action']=='update'){
												$valueProductOption['product_id']=$this->model_extension_purpletree_multivendor_bulkproductupload->checkExistingProductIds($commonData['column_name'],$filterProData[$valueProductOption['product_id']][$commonData['column_name']]);	
											}
...
$status_msgg[] = array("status"=>"success", "msg" => "Product  ID ".$product_id." product option value Data Updated Successfully");
		...
		protected function getData() {
			...
			$data['helplink'] = "https://cutt.ly/bCo8tKV";
			...
			'href' => $this->url->link('extension/account/purpletree_multivendor/bulkproductupload', '' . $url, true)
			);	
			$this->load->model('extension/purpletree_multivendor/dashboard');
			$this->model_extension_purpletree_multivendor_dashboard->checkSellerApproval();
			...
			$data['lang']=$this->model_extension_purpletree_multivendor_bulkproductupload->getAllLanguage();
			$data['seller_template'] = HTTPS_SERVER."image/catalog/seller_purpletree_bulk_product_upload.xlsx";
			$data['next_product_id'] = 1;
			$data['seller_id']=$this->model_extension_purpletree_multivendor_bulkproductupload->getAllSellerId();
			$lastproductid = $this->model_extension_purpletree_multivendor_bulkproductupload->getlastproductId();
			if(isset($lastproductid)) {
				$data['next_product_id'] = $lastproductid+1;
			}
			$this->response->setOutput($this->load->view('account/purpletree_multivendor/bulkproduct_upload', $data));		
		}
	}
