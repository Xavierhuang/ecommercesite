...
class ControllerExtensionAccountPurpletreeMultivendorSellerproduct extends Controller{
		...
		public function add() {
			...
			if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
				...
				if($this->validateForm()) {
					...
					$this->response->redirect($this->url->link('extension/account/purpletree_multivendor/sellerproduct',$url, true));
				}
			}
		...
		protected function getForm() {
			...
			if (!isset($this->request->get['product_id'])) {
				$data['action'] = $this->url->link('extension/account/purpletree_multivendor/sellerproduct/add',$url,true);
				} else {
				$data['action'] = $this->url->link('extension/account/purpletree_multivendor/sellerproduct/edit','&product_id=' . $this->request->get['product_id'] .$url, true);
			}
			$seller_id = $this->customer->getId();
			$data['cancel'] = $this->url->link('extension/account/purpletree_multivendor/sellerproduct',$url, true);
			...
			$data['quick_order_check'] ='';
			if (defined('QUICK_ORDER') && QUICK_ORDER == 1 ){
			     $data['quick_order_check'] = QUICK_ORDER;
				}
			if($data['quick_order_check'] == 1){
			if (isset($this->request->post['product_name'])) {
				$data['product_name'] = $this->request->post['product_description'];
				} elseif (isset($this->request->get['product_id'])) {
				$data['product_name'] = $data['product_description'][$this->config->get('config_language_id')]['name'];
				} else {
				$product_name = 'Delivery'.date('d-m-y');
				$data['product_name'] = str_replace("-","",$product_name);
				
			}
			if (isset($this->request->post['delivery_address'])) {
				$data['delivery_address'] = $this->request->post['delivery_address'];
				} elseif (!empty($product_info)) {
				$data['delivery_address'] = $product_info['delivery_address'];
				} else {
				$data['delivery_address'] = '';
			}
			if (isset($this->request->post['deliveraddresslat'])) {
				$data['deliveraddresslat'] = $this->request->post['deliveraddresslat'];
				} elseif (!empty($product_info)) {
				$data['deliveraddresslat'] = $product_info['deliveraddresslat'];
				} else {
				$data['deliveraddresslat'] = '';
			}
			if (isset($this->request->post['deliveraddresslon'])) {
				$data['deliveraddresslon'] = $this->request->post['deliveraddresslon'];
				} elseif (!empty($product_info)) {
				$data['deliveraddresslon'] = $product_info['deliveraddresslon'];
				} else {
			...
			if (isset($this->request->get['product_id'])) {
				$quick_order_status = $this->model_extension_purpletree_multivendor_sellerproduct->getQucikOrderStatus($this->request->get['product_id']);
			}
		...
		//****** End pts shipping *******//
			...
			$restrict_products_status = $this->model_extension_purpletree_multivendor_sellerproduct->getModificationsForRestrictProduct();
			
			$data['restrict_products_status'] = $restrict_products_status;
			if(isset($restrict_products_status)){
				$this->model_extension_purpletree_multivendor_sellerproduct->CreateCatalogTableProduct();
				if (isset($this->request->post['product_to_customer_group'])) {
					...
					$data['product_to_customer_group'] = $this->model_extension_purpletree_multivendor_sellerproduct->getProductByCustomerGroups($this->request->get['product_id']);
					} else {
		...
		protected function getList(){
			...
			'value' => 'ASC',
			'href'  => $this->url->link('extension/account/purpletree_multivendor/sellerproduct', '&order=ASC' . $url,true)
			);
			
			$data['sorts_order'][] = array(
			'text'  => $this->language->get('text_name_desc'),
			'value' => 'DESC',
			'href'  => $this->url->link('extension/account/purpletree_multivendor/sellerproduct','&order=DESC' . $url,true)
			);
           ...
           $data['languages'] = $this->model_localisation_language->getLanguages();
			...
			foreach ($results as $result) {
				...
				$price = $this->currency->format(($result['price'] + $price_extra),  $this->session->data['currency']);
...
$store_detail = $this->customer->isSeller();
		...
		public function quickEdit() {
			...
			if (isset($this->request->get['product_id'])) {
				...
				//$results = $this->model_extension_purpletree_multivendor_sellerproduct->getProducts($filter_data);
				$seller_id = $this->customer->getId();
				$result = $this->model_extension_purpletree_multivendor_sellerproduct->getProduct($this->request->get['product_id'],$seller_id);
      ...
      foreach ($product_specials as $product_special) {
			...
			$product_description = $this->model_extension_purpletree_multivendor_sellerproduct->getProductDescriptions($this->request->get['product_id']);
		...
		public function quickSave() {
		  ...
		  if (!$this->customer->isLogged()) {
			...
			if($this->request->post['quick_product_id']){
			    $this->model_extension_purpletree_multivendor_sellerproduct->qucikEditProduct($this->request->post['quick_product_id'],$this->request->post);
			    $currentproductname = $this->model_extension_purpletree_multivendor_sellerproduct->getproductname($this->request->post['quick_product_id']);
