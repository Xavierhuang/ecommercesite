...
class ControllerExtensionAccountPurpletreeMultivendorBlogPost extends Controller{
		...
		public function index(){ 
			...
			if(isset($this->request->get['blog_post_id'])){
				$seller_storessst = $this->model_extension_module_purpletree_sellerblog->checkSellerVacation($this->request->get['blog_post_id']);
		...
		public function all_blog(){    
			...
			if ($results) {
				...
				foreach($results as $result) {								  
					...
					'thumb'       => $image,
					'title'        => $result['title'],
					'description' => utf8_substr(utf8_substr(strip_tags(html_entity_decode($result['description'], ENT_QUOTES, 'UTF-8')), 0, $this->config->get('theme_' . $this->config->get('config_theme') . '_product_description_length')), 0, 20) . '...',
				...
				$pagination->url = $this->url->link('extension/account/purpletree_multivendor/blog_post/all_blog', '&page={page}');
