...
class ControllerExtensionAccountPurpletreeMultivendorShipping extends Controller {
		...
		public function edit() {
			...
			if (($this->request->server['REQUEST_METHOD'] == 'POST' && $this->validateForm()) ) {
				$seller_id=$this->customer->getId();
				$this->model_extension_purpletree_multivendor_shipping->editShipping($seller_id,$this->request->get['shipping_id'],$this->request->post);
		...
		protected function getList() {
			...
			foreach ($results as $result) {
				...
				'edit'           => $this->url->link('extension/account/purpletree_multivendor/shipping/edit', '' . 'shipping_id=' . $result['id'] . $url, true)
				);
			}
