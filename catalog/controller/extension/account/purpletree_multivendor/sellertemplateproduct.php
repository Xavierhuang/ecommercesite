...
class ControllerExtensionAccountPurpletreeMultivendorSellertemplateproduct extends Controller{
		...
		public function edit() {
			...
			if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
				...
				if($this->validateForm()) {
					...
					if(isset($this->request->get['template_id'])){
					$this->request->post['product_id'] =$this->model_extension_purpletree_multivendor_sellertemplateproduct->getProductId($this->request->get['template_id']);
					}
			  ...
			  if($this->request->post['status'] == 1){
			    $this->db->query("UPDATE " . DB_PREFIX . "product SET status = '" . (int)$this->request->post['status'] . "' WHERE product_id = '" . (int)$temp_product_id . "'");
				  ...
				  $temp_status = $this->model_extension_purpletree_multivendor_sellertemplateproduct->checkTempstatus($temp_product_id);		
			     if($temp_status == 1){
			      $status1 = 1;
		...
		protected function getForm() {
			...
			$product_options = $this->model_extension_purpletree_multivendor_sellertemplateproduct->getTempProductOptions($this->request->get['id']);
		...
		$data['product_id'] = '';
		if(isset($this->request->get['template_id'])){
		$data['product_id'] = $this->model_extension_purpletree_multivendor_sellertemplateproduct->getProductId($this->request->get['template_id']);
		}
			...
			$data['option_values'][$product_option['option_id']] = $this->model_extension_purpletree_multivendor_sellertemplateproduct->getOptionValuesByProductOptId($product_option['option_id'],$data['product_id']);
			}
			}
			}
		...
		public function sellerTemplateOption() {
			...
			if (isset($this->request->get['filter_name'])) {
				...
				$results = $this->model_extension_purpletree_multivendor_sellertemplateproduct->getTemplateProductStatus($product_id,$option['option_id']);
				if(!empty($results) && ($option['type'] == 'select' || $option['type'] == 'radio')){
					...
					if ($option['type'] == 'select' || $option['type'] == 'radio' || $option['type'] == 'checkbox' || $option['type'] == 'image') {
						...
						foreach ($option_values as $option_value) {
							...
							$checkOptions = $this->model_extension_purpletree_multivendor_sellertemplateproduct->getTemplateProductOption($data);
							if($checkOptions){
