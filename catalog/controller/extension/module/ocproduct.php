	...
	public function index($setting) {
		...
		if ($results) {
			foreach ($results as $result) {
				...
				$sold_number = $this->model_extension_module_ocproduct->getSoldNumber($result['product_id']);
				...
				foreach ($img_results as $img) {
					$product_images[] = array(
						'main' => $this->model_tool_image->resize($img['image'], $setting['width'], $setting['height']),
				...
				$data['products'][] = array(
					...
					'model'       => $model,
					'name'        => $result['name'],
					'description' => utf8_substr(trim(strip_tags(html_entity_decode($result['description'], ENT_QUOTES, 'UTF-8'))), 0, $setting['des_limit']) . '..',
					'price'       => $price,
					'special'     => $special,
