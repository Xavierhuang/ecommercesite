...
class ControllerExtensionModuleOcTabProducts extends Controller {
	public function index($setting) {
		...
		if(isset($setting['module_image_width'])) {
			$module_image_width = (int) $setting['module_image_width'];
		} else {
			$module_image_width = 200;
		}
		if(isset($setting['module_image_height'])) {
			$module_image_height = (int) $setting['module_image_height'];
		} else {
			$module_image_height = 200;
		}
		if(isset($setting['module_image'])) {
			$data['module_image'] = $this->model_tool_image->resize($setting['module_image'], $module_image_width, $module_image_height);
		} else {
		...
		if(isset($setting['module_image_width2'])) {
			$module_image_width2 = (int) $setting['module_image_width2'];
		} else {
			$module_image_width2 = 200;
		}
		if(isset($setting['module_image_height2'])) {
			$module_image_height2 = (int) $setting['module_image_height2'];
		} else {
			$module_image_height2 = 200;
		}
		if(isset($setting['module_image2'])) {
			$data['module_image2'] = $this->model_tool_image->resize($setting['module_image2'], $module_image_width2, $module_image_height2);
		} else {
		...
		foreach($setting['octab'] as $octab){
			...
			if($octab['option'] == 0) {
				...
				if($octab['cate_id']) {
					...
					if(isset($category_info['thumbnail_image']) && $category_info['thumbnail_image'] != '') {
                        $thumbnail_image = $this->model_tool_image->resize($category_info['thumbnail_image'], 120, 74);
                    } else {
                        $thumbnail_image = $this->model_tool_image->resize('placeholder.png', 120, 74);
                    }
			...
			if ($excp_frs_products) {
				foreach ($excp_frs_products as $result) {
					...
					$sold_number = (int) $this->model_extension_module_ocproduct->getSoldNumber($result['product_id']);
					$excp_frs_products_data[] = array(
						'product_id'  => $result['product_id'],
						'thumb'       => $image,
						'model'       => $model,
