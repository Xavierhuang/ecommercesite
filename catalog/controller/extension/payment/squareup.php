...
class ControllerExtensionPaymentSquareup extends Controller {
    public function index() {
        ...
        if($this->config->get('payment_squareup_key_type') == 'sandbox'){
          $data['squareup_js_api'] = Squareup::SANDBOX_PAYMENT_FORM_URL;
        }
        ...
        if (is_null($location_currency)) {
            ...
            if ($this->session->data['currency'] != $location_currency) {
              ...
              $amount = round($this->currency->convert($this->cart->getTotal(), $this->config->get('config_currency'), $location_currency), (int)$dec_place);
    ...
    public function saveCard(){
      ...
      try {
          ...
          if (empty($this->request->post['squareup_digital_wallet_type']) || $this->request->post['squareup_digital_wallet_type'] == 'NONE') {
              if ($this->customer->isLogged() && isset($this->request->post['squareup_save_card'])) {
                  // Save the card
                  $card_data = array(
    ...
    public function checkout() {
        ...
        try {
            ...
            if (empty($this->request->post['squareup_digital_wallet_type']) || $this->request->post['squareup_digital_wallet_type'] == 'NONE') {
                ...
                if ($this->customer->isLogged() && isset($this->request->post['squareup_save_card']) && strlen($this->request->post['squareup_save_card']) > 10) {
                  ...
                  $use_saved = true;
                  $square_card_id = $this->request->post['squareup_save_card'];
                }
            }
            ...
            if ($use_saved) {
                ...
                $transaction_data['verification_token'] = $this->request->post['squareup_buyer_verification_token'];
                ...
                $transaction_data['source_id'] = $this->request->post['squareup_nonce'];
                $transaction_data['verification_token'] = $this->request->post['squareup_buyer_verification_token'];
