...
class ModelExtensionPaymentSquareup extends Model {
    ...
    public function oneCronStandardPeriodHasPassed() {
        ...
        $time = (int)$this->config->get('payment_squareup_cron_standard_started_at');
    ...
    public function oneCronInventoryPeriodHasPassed() {
        ...
        $time = (int)$this->config->get('payment_squareup_cron_inventory_started_at');
    ...
    public function setBeginCronFlags($flag) {
        ...
        $this->editSquareSetting(array(
            'payment_squareup_cron_is_running' => '1',
            'payment_squareup_cron_started_at' => $now,
        ...
        switch ($flag) {
            case self::CRON_STARTED_FLAG_STANDARD:
                $this->editSquareSetting(array(
                    'payment_squareup_cron_standard_started_at' => $now,
                ));
                break;
            case self::CRON_STARTED_FLAG_INVENTORY:
                $this->editSquareSetting(array(
                    'payment_squareup_cron_inventory_started_at' => $now,
                ));
                break;
        }
    ...
    public function getApplePayLineItems($order_id) {
        ...
        foreach ($this->model_checkout_order->getOrderProducts($order_id) as $product) {
            ...
            $lineItems[] = array(
                ...
                'amount' => $this->convertPriceNumeric($product['total'], $order_info['currency_code']),
                'pending' => false
            );
        }
        ...
        foreach ($this->model_checkout_order->getOrderTotals($order_id) as $total) {
            if ($total['code'] != 'sub_total' && $total['code'] != 'total') {
                $lineItems[] = array(
                    'label' => $total['title'],
                    'amount' => $this->convertPriceNumeric($total['value'], $order_info['currency_code']),
                    'pending' => false
                );
    ...
    private function convertPriceNumeric($price, $currency) {
        return (string)$this->currency->format($this->currency->convert((float)$price, $this->config->get('config_currency'), $currency), $currency, '', false);
    }
