...
class ModelExtensionModuleStorefeatured extends Model {
		public function getLatest() {
			...
			$query = $this->db->query("SELECT pvs.*,(SELECT CONCAT(c.firstname, ' ', c.lastname) FROM " . DB_PREFIX . "customer c WHERE c.customer_id = pvs.seller_id) AS seller FROM " . DB_PREFIX . "purpletree_vendor_stores pvs LEFT JOIN " . DB_PREFIX . "purpletree_vendor_plan_subscription pvps ON (pvps.seller_id =pvs.seller_id) LEFT JOIN " . DB_PREFIX . "purpletree_vendor_seller_plan pvsp ON (pvsp.seller_id =pvs.seller_id) LEFT JOIN " . DB_PREFIX . "purpletree_vendor_plan pvp ON (pvp.plan_id =pvsp.plan_id) RIGHT JOIN " . DB_PREFIX . "customer c  ON (pvs.seller_id = c.customer_id) WHERE pvps.status_id = 1 AND pvsp.".$status."=1 AND pvp.featured_store =1 AND pvs.vacation = '0' AND FIND_IN_SET('".$store_id."',pvs.multi_store_id) AND pvs.multi_store_id='".(int)$this->config->get('config_store_id') ."'  AND pvs.store_status='1' GROUP BY pvs.seller_id ORDER BY pvs.sort_order "  );
