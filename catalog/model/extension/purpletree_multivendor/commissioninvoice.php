...
class ModelExtensionPurpletreeMultivendorCommissioninvoice extends Model{
		...
		public function getCommissions($data=array()){
			$sql = "SELECT pvc.*,pvc.created_at FROM " . DB_PREFIX . "purpletree_vendor_commission_invoice pvc JOIN " . DB_PREFIX . "purpletree_vendor_commission_invoice_items pvci ON (pvci.link_id = pvc.id) RIGHT JOIN " . DB_PREFIX . "order o ON(o.order_id = pvci.order_id) WHERE  pvci.seller_id ='".(int)$data['seller_id']."'";
