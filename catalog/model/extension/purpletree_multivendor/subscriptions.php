...
class ModelExtensionPurpletreeMultivendorSubscriptions extends Model {	
		...
		public function getSubscriptionPlanStatus($status) {
			$sql = "SELECT pvpisl.status_id,pvpisl.status FROM " . DB_PREFIX . "purpletree_vendor_plan_invoice_status_l pvpisl WHERE pvpisl.status  LIKE '%" . $this->db->escape($status) . "%'AND pvpisl.language_id = '" . (int)$this->config->get('config_language_id') . "'";
			$query = $this->db->query($sql);
			return $query->rows;	
		}
