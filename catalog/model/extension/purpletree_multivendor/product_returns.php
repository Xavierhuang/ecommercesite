...
class ModelExtensionPurpletreeMultivendorProductReturns extends Model {
		...
		public function getTotalReturns($data = array()) {
			$sql = "SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "return` r JOIN " . DB_PREFIX . "purpletree_vendor_orders pov ON (pov.order_id = r.order_id ) JOIN " . DB_PREFIX . "purpletree_vendor_products pvp ON (pvp.product_id = r.product_id AND pvp.seller_id='".(int)$this->customer->getId()."' )";	
			$implode = array();
			$implode[] = " pov.seller_id = '" . (int)$this->customer->getId(). "'";
			if (!empty($data['filter_return_id'])) {
				$implode[] = "r.return_id = '" . (int)$data['filter_return_id'] . "'";
		...
		public function addReturnHistory($return_id, $return_status_id, $comment, $notify) {
			...
			$this->db->query("INSERT INTO `" . DB_PREFIX . "return_history` SET `return_id` = '" . (int)$return_id . "',`seller_id`='".(int)$this->customer->getId()."',`return_status_id` = '" . (int)$return_status_id . "', `notify` = '" . (int)$notify . "', `comment` = '" . $this->db->escape(strip_tags($comment)) . "', `date_added` = NOW()");
			if ($notify) {
