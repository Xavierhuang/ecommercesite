...
class ModelExtensionPurpletreeMultivendorSellerorder extends Model{
	//module_purpletree_product_designer_status
				...
				if (!$order_info['order_status_id'] && $order_status_id) {
					...
					foreach ($order_product_query->rows as $product) {
						...
						foreach ($order_option_query->rows as $option) {
							...
							$product_detail .= chr(9) . '-' . $option['name'] . ' ' . (utf8_strlen($value) > 20 ? utf8_substr($value, 0, 20) . '..' : $value) . "\n";
						}
					}
					$product_desc='';
					foreach ($order_voucher_query->rows as $voucher) {
		...
		if($comment_ins!=''){
			if($order_info_url!=''){
				if($order_down_url!=''){
					if($p_comment!=''){
						$getTemplate = $this->model_extension_purpletree_multivendor_vendor->getSelleRegisterEmailTemplate('seller_product_mail_to_cutomer_condition_first');
						
					} else {
						$getTemplate = $this->model_extension_purpletree_multivendor_vendor->getSelleRegisterEmailTemplate('front_seller_prodcut_mail_to_seller_condition_second');
					
					}
				} else {
					if($p_comment!=''){
						$getTemplate = $this->model_extension_purpletree_multivendor_vendor->getSelleRegisterEmailTemplate('seller_product_mail_to_cutomer_condition_third');
						
					} else {
						$getTemplate = $this->model_extension_purpletree_multivendor_vendor->getSelleRegisterEmailTemplate('seller_product_mail_to_cutomer_condition_fourth');
			...
			if($order_info_url!=''){
				if($order_down_url!=''){
					if($p_comment!=''){
						$getTemplate = $this->model_extension_purpletree_multivendor_vendor->getSelleRegisterEmailTemplate('seller_product_mail_to_cutomer_condition_nine');
						
					} else {
						$getTemplate = $this->model_extension_purpletree_multivendor_vendor->getSelleRegisterEmailTemplate('seller_product_mail_to_cutomer_condition_ten');
					
					}
				} else {
					if($p_comment!=''){
						$getTemplate = $this->model_extension_purpletree_multivendor_vendor->getSelleRegisterEmailTemplate('seller_product_mail_to_cutomer_condition_eleven');
						
					} else {
						$getTemplate = $this->model_extension_purpletree_multivendor_vendor->getSelleRegisterEmailTemplate('seller_product_mail_to_cutomer_condition_twelve');
				...
				if($order_down_url!=''){
					if($p_comment!=''){
						$getTemplate = $this->model_extension_purpletree_multivendor_vendor->getSelleRegisterEmailTemplate('seller_product_mail_to_cutomer_condition_thirteen');
						
					} else {
						$getTemplate = $this->model_extension_purpletree_multivendor_vendor->getSelleRegisterEmailTemplate('seller_product_mail_to_cutomer_condition_fourteen');
					
					}
				} else {
					if($p_comment!=''){
						$getTemplate = $this->model_extension_purpletree_multivendor_vendor->getSelleRegisterEmailTemplate('seller_product_mail_to_cutomer_condition_fifteen');
						
					} else {
						$getTemplate = $this->model_extension_purpletree_multivendor_vendor->getSelleRegisterEmailTemplate('seller_product_mail_to_cutomer_condition_sixteen');
		...
		$getTemplate = $this->model_extension_purpletree_multivendor_vendor->getSelleRegisterEmailTemplate('front_seller_order_update_condition_first_by_seller');
		}
		...
		$this->load->model('extension/purpletree_multivendor/vendor');	
			$getTemplate='';	
			if($p_comment!=''){
			$getTemplate = $this->model_extension_purpletree_multivendor_vendor->getSelleRegisterEmailTemplate('front_seller_prodcut_mail_to_seller_condition_first');		
			} else {
			$getTemplate = $this->model_extension_purpletree_multivendor_vendor->getSelleRegisterEmailTemplate('front_seller_prodcut_mail_to_seller_condition_second');	
			}		
		...
		if($p_comment!=''){
		$getTemplate = $this->model_extension_purpletree_multivendor_vendor->getSelleRegisterEmailTemplate('front_seller_prodcut_alert_mail_condition_first');		
		} else {
		$getTemplate = $this->model_extension_purpletree_multivendor_vendor->getSelleRegisterEmailTemplate('front_seller_prodcut_alert_mail_condition_second');	
		}
		...
		$emails = explode(',', $this->config->get('config_alert_email'));
				...
				foreach ($emails as $email) {
					if ($email && filter_var($email, FILTER_VALIDATE_EMAIL)) {
						$this->model_extension_purpletree_multivendor_vendor->ptsSendMail($email,$subject,$message);
						}
					}
	...
	if($p_order_info!=''){		
		if($p_order_status!='' && $p_comment!=''){	
		$getTemplate = $this->model_extension_purpletree_multivendor_vendor->getSelleRegisterEmailTemplate('front_customer_notify_on_order_update_condition_first_by_seller');
		}
		
		if($p_order_status!='' && $p_comment==''){	
		$getTemplate = $this->model_extension_purpletree_multivendor_vendor->getSelleRegisterEmailTemplate('front_customer_notify_on_order_update_condition_second_by_seller');
		}
					
		if($p_order_status=='' && $p_comment!=''){
			$getTemplate = $this->model_extension_purpletree_multivendor_vendor->getSelleRegisterEmailTemplate('front_seller_order_update_notify_condition_third_by_seller');
		}
					
		if($p_order_status=='' && $p_comment==''){
			$getTemplate = $this->model_extension_purpletree_multivendor_vendor->getSelleRegisterEmailTemplate('front_seller_order_update_notify_condition_fourth_by_seller');
		}
	} else {
		if($p_order_status!='' && $p_comment!=''){	
		$getTemplate = $this->model_extension_purpletree_multivendor_vendor->getSelleRegisterEmailTemplate('front_customer_notify_on_order_update_condition_fifth_by_seller');
		}
		
		if($p_order_status!='' && $p_comment==''){	
		$getTemplate = $this->model_extension_purpletree_multivendor_vendor->getSelleRegisterEmailTemplate('front_customer_notify_on_order_update_condition_sixth_by_seller');
		}
					
		if($p_order_status=='' && $p_comment!=''){
			$getTemplate = $this->model_extension_purpletree_multivendor_vendor->getSelleRegisterEmailTemplate('front_customer_notify_on_order_update_condition_seventh_by_seller');
		}
		...
		if($p_order_status!='' && $p_comment==''){	
		$getTemplate = $this->model_extension_purpletree_multivendor_vendor->getSelleRegisterEmailTemplate('front_seller_order_update_condition_second_by_seller');
		}
					
		if($p_order_status=='' && $p_comment!=''){
			$getTemplate = $this->model_extension_purpletree_multivendor_vendor->getSelleRegisterEmailTemplate('front_seller_order_update_condition_third_by_seller');
		}
					
		if($p_order_status=='' && $p_comment==''){
			$getTemplate = $this->model_extension_purpletree_multivendor_vendor->getSelleRegisterEmailTemplate('front_seller_order_update_condition_fourth_by_seller');
		}
			$mail_subject_vars=array(
								'_ORDER_ID_'=>$order_id,
								'_STORE_NAME_'=>$order_store_name
