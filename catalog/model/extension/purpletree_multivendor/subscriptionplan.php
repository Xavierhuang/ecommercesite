...
class ModelExtensionPurpletreeMultivendorSubscriptionplan extends Model{
		...
		public function getstausfromid($id) {
			$query = $this->db->query("SELECT pvpisl.status FROM `".DB_PREFIX."purpletree_vendor_plan_invoice_status` pvpis LEFT JOIN `".DB_PREFIX."purpletree_vendor_plan_invoice_status_l` pvpisl ON (pvpisl.status_id = pvpis.status_id) WHERE pvpisl.language_id='". (int)$this->config->get('config_language_id') ."' AND pvpisl.status_id ='". $id ."'");
			if($query->num_rows){
		...
		public function getstatuslist() {
			$query = $this->db->query("SELECT pvpis.status_id,pvpisl.status FROM `".DB_PREFIX."purpletree_vendor_plan_invoice_status` pvpis LEFT JOIN `".DB_PREFIX."purpletree_vendor_plan_invoice_status_l` pvpisl ON (pvpisl.status_id = pvpis.status_id) WHERE pvpisl.language_id='". (int)$this->config->get('config_language_id') ."'");
			if($query->num_rows){
				return $query->rows;	
				} else {
				return NUll;		
	...
	public function getSubscribePlanDetail($plan_id,$invoice_id) {
			$sql="SELECT pvsp.start_date ,pvsp.end_date,pvp.plan_id,pvp.no_of_product,pvp.joining_fee,pvp.subscription_price,pvp.validity,pvp.created_date,pvp.modified_date,pvpd.plan_name,pvpd.plan_description,pvpd.plan_short_description  FROM ". DB_PREFIX ."purpletree_vendor_plan pvp LEFT JOIN ". DB_PREFIX ."purpletree_vendor_plan_description pvpd ON (pvp.plan_id=pvpd.plan_id) LEFT JOIN ". DB_PREFIX ."purpletree_vendor_seller_plan pvsp ON (pvp.plan_id=pvsp.plan_id) WHERE pvpd.language_id='".(int)$this->config->get('config_language_id') ."' AND pvp.plan_id='".(int)$plan_id."' AND pvsp.invoice_id='".(int)$invoice_id."'";
        ...
        public function getPlanDetail($plan_id) {
            $query=$this->db->query("SELECT * FROM ". DB_PREFIX ."purpletree_vendor_plan WHERE plan_id='".$plan_id."' AND status=1");
            if($query->num_rows){
                return $query->row;
                } else {
                return NULL;
