					...
					if(json) {
						...
						$("[onclick*='cart.add']").each(function() {
							...
							if(json[product_id] && target.length) {
								...
								target.find('.btn-1-extra').attr('onclick', 'logprcatc.dologin();').html(json['btntxt']);
							}
						});	
					}
				}
			...
			$.ajax({
				...
				success: function(json) {
					...
					if(json && json[product_id]) {						
						...
						target.find('.btn-1-extra').unbind('click').html(json['btntxt']).attr('onclick', 'logprcatc.dologin();').removeAttr('id');
					}					
				}
			});
 		}
